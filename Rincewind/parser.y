...
%type<statement>  script_statement condition jump option script dialog statement
...
|				"-" TEXT ":" jump				{ $$ = CreateDialog(CreateUniqueString($2), $4); 						}
...
|				"-" TEXT ":" condition jump		{ $$ = CreateCondition($4, CreateDialog(CreateUniqueString($2), $5)); }
...
option:			"*" jump						{ $$ = CreateOption($2);												}
|				"*" TEXT						{ $$ = CreateOption(CreateString($2));									}
|				"*" jump condition				{ $$ = CreateCondition($3, CreateOption($2));							}
|				"*" TEXT condition				{ $$ = CreateCondition($3, CreateOption(CreateString($2)));				}
;


jump:			"[" TEXT "]" "(" LABEL ")"		{ $$ = CreateJump(CreateString($2), CreateLabel($5)); 					}
;
